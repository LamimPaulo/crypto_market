  ...
  getListedCoinTradeHistory() async {
    ...
    for (var i = 0; i < data['data'].length; i++) {
      tradeHistoryList.add(TradeHistory(
          date: data['data'][i]['T'] == null
              ? ""
              : DateTime.fromMillisecondsSinceEpoch(
                      int.parse(data['data'][i]['T'].toString()))
                  .toString()
                  .split(' ')[1],
          type: data['data'][i]['m'] == true ? "Buy" : "Sell",
          price: data['data'][i]['p'] == null
              ? ""
              : double.parse(data['data'][i]['p'].toString()).toString(),
          amount: data['data'][i]['q'] == null
              ? ""
              : data['data'][i]['q'].toString()));
    }
    setState(() {});
  }
                    ...
                    return Padding(
                      ...
                      child: SizedBox(
                          ...
                          child: ListView.builder(
                              ...
                              primary: false,
                              reverse: true,
                              itemCount: TradeHistoryController
                                  .to.tradeHistoryList.length,
                              // itemCount: coinTradeHistoryList.length < 10 ? coinTradeHistoryList.length : 10,
                              itemBuilder: (BuildContext ctx, int i) {
                                // return _orderHistory(coinTradeHistoryList[i]);
                                return _orderHistory(TradeHistoryController
                                    .to.tradeHistoryList[i]);
                              })),
                    );
                    // }
                    // else {
